...
def speed_bouts(speed_data, time):
    ...
    for fr in range(0, len(speed_data) - 5):
        ...
        if speed_data[fr] > 1 and np.mean(speed_data[fr - 5:fr - 1]) < np.mean(speed_data[fr + 1:fr + 5]):
            acc_bouts.append(fr)
...
def speed_data():
    ...
    nospeed_duration_ctrl_all = []
    lospeed_duration_ctrl_all = []
    midspeed_duration_ctrl_all = []
    hispeed_duration_ctrl_all = []
    acc_duration_ctrl_all = []
    nospeed_duration_amph_all = []
    lospeed_duration_amph_all = []
    midspeed_duration_amph_all = []
    hispeed_duration_amph_all = []
    ...
    for drug in drugs:
        ...
        nospeed_duration_ctrl_perdrug = np.nanmean(nospeed_duration_ctrl_peranimal, axis=0)
        lospeed_duration_ctrl_perdrug = np.nanmean(lospeed_duration_ctrl_peranimal, axis=0)
        midspeed_duration_ctrl_perdrug = np.nanmean(midspeed_duration_ctrl_peranimal, axis=0)
        hispeed_duration_ctrl_perdrug = np.nanmean(hispeed_duration_ctrl_peranimal, axis=0)
        acc_duration_ctrl_perdrug = np.nanmean(acc_duration_ctrl_peranimal, axis=0)
        nospeed_duration_amph_perdrug = np.nanmean(nospeed_duration_amph_peranimal, axis=0)
        lospeed_duration_amph_perdrug = np.nanmean(lospeed_duration_amph_peranimal, axis=0)
        midspeed_duration_amph_perdrug = np.nanmean(midspeed_duration_amph_peranimal, axis=0)
        hispeed_duration_amph_perdrug = np.nanmean(hispeed_duration_amph_peranimal, axis=0)
        acc_duration_amph_perdrug = np.nanmean(acc_duration_amph_peranimal, axis=0)

        nospeed_duration_ctrl_all.append(nospeed_duration_ctrl_perdrug)
        lospeed_duration_ctrl_all.append(lospeed_duration_ctrl_perdrug)
        midspeed_duration_ctrl_all.append(midspeed_duration_ctrl_perdrug)
        hispeed_duration_ctrl_all.append(hispeed_duration_ctrl_perdrug)
        acc_duration_ctrl_all.append(acc_duration_ctrl_perdrug)
        nospeed_duration_amph_all.append(nospeed_duration_amph_perdrug)
        lospeed_duration_amph_all.append(lospeed_duration_amph_perdrug)
        midspeed_duration_amph_all.append(midspeed_duration_amph_perdrug)
        hispeed_duration_amph_all.append(hispeed_duration_amph_perdrug)
...
def plot():
    ...
    plt.subplot(211)
    plt.bar('speed_ctrl', nospeed_duration_ctrl, yerr=nospeed_duration_ctrl_sem)
    plt.bar('speed_ctrl', lospeed_duration_ctrl, yerr=lospeed_duration_ctrl_sem)
    plt.bar('speed_ctrl', midspeed_duration_ctrl, yerr=midspeed_duration_ctrl_sem)
    plt.bar('speed_ctrl', hispeed_duration_ctrl, yerr=hispeed_duration_ctrl_sem)
    plt.bar('acc_ctrl', acc_duration_ctrl, yerr=acc_duration_ctrl_sem)
    plt.bar('speed_amph', nospeed_duration_amph, yerr=nospeed_duration_amph_sem)
    plt.bar('speed_amph', lospeed_duration_amph, yerr=lospeed_duration_amph_sem)
    plt.bar('speed_amph', midspeed_duration_amph, yerr=midspeed_duration_amph_sem)
    plt.bar('speed_amph', hispeed_duration_amph, yerr=hispeed_duration_amph_sem)
