...
def generate_alldata():
    ...
    for drug in drugs:
        for dose in doses:
            ...
            data_ctrl, data_amph = get_stats(drug, dose)
            for animal in animals:
                if dose == 'Vehicle':
                    for k in data_ctrl.keys():
                        alldata[dose][drug][animal][k].append(data_ctrl[k])
                    for k in data_amph.keys():
                        alldata[dose][drug][animal][k].append(data_amph[k])
                else:
...
def plot():
    ...
    data_ctrl_mean, data_ctrl_sem, data_amph_mean, data_amph_sem = get_stats()
