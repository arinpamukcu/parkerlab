        ...
        while line != '':
            ...
            while '---' not in line:
                ...
                if all('.' not in s for s in start_stop_duration):
                    ...
                    bout_start = max((np.where(np.append(timestamps, np.inf) >= float(start_stop_duration[0]))[0][0],
                                      start_frame - 1))
                    bout_end = min((np.where(np.append(timestamps, np.inf) >= float(start_stop_duration[1]))[0][0],
                ...
                behaviors_boutwise[curr_behavior] = np.vstack(
                    (behaviors_boutwise[curr_behavior], np.array([bout_start, bout_end], int)))
...
def merge_channels(channel_dict, use_channels, omit_channels, end_frame, target_behaviors=[]):
    ...
    for ch in use_channels:
        if (ch in channel_dict):
            ...
            if not (behFlag):
                changed_behavior_list = [annotated_behavior if annotated_behavior in keep_behaviors else 'other' for
                                         annotated_behavior in
                                         chosen_behavior_list]
                behFlag = 1
            else:
                changed_behavior_list = [anno[0] if anno[1] not in keep_behaviors else anno[1] for anno in
                                         zip(changed_behavior_list, chosen_behavior_list)]
        else:
